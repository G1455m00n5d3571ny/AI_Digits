...
# Виды ошибок
      ...
      # 4. False Negative(FN) - ложно-отрицтельный
...
# Критерии отчета
      ...
      # из всех предсказанных чисел "5", сколько реално было "5"?
...
print(classification_report(
    true_target_classes, # истинные выходные данные
    predicted_classes, # предсказанные классы
    target_names = [str(i) for i in range(10)] # наименовния чисел статистики (0-9)
))
...
for i in range(20):
    ax = axes[i // 5, i % 5]
    ax.imshow(data_test[i], cmap = 'gray') # отрисовка картинки
...
plt.subtitle('Примеры предсказаний (зеленый - верно, красный - неверно)', fontsize = 14, fontweight = 'bold')
plt.tight_layout()
...
plt.subtitle('20 самых уверенных ошибок', fontsize = 14, fontweight = 'bold')
plt.tight_layout()
